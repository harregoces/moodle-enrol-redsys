	...
	*/
	...
	if(!class_exists('ISSignatureUtils')){
		class ISSignatureUtils{
			...
			static function encodeBase64($data){
			...
			static function decodeBase64($data){
			...
			static function getOrder($datos){
			...
			static function createMerchantSignatureHostToHost($key, $ent){
			...
			static function createMerchantSignatureResponseHostToHost($key, $datos, $numPedido){
				// Se decodifica la clave Base64
				$key = ISSignatureUtils::decodeBase64($key);
				// Se diversifica la clave con el Número de Pedido
				$key = ISSignatureUtils::encrypt_3DES($numPedido, $key);
